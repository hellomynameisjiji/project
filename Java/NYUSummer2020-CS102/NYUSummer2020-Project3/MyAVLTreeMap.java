...
public class MyAVLTreeMap<K,V> extends TreeMap<K,V> {
  ...
  public void printTree() {
	  ...
	  String[][] treePicture =  new String[100][100];

	  printTree1(0, 50, 0, tree.root(),  treePicture);
	  
	  System.out.println("Print of tree");
		 for (int i = 0; i < 100; i++) {
			 for (int j = 0; j < 100; j++) {
				 if (treePicture[i][j] == null) {
					 System.out.print(" ");
				 } else {
					 System.out.print(treePicture[i][j]);					 
				 }
			 	}
			 System.out.println();
			 }
  ...
  public void printTree1(int row, int col,int depth, Position<Entry<K, V>> p, String[][] treePicture) {
	  treePicture[row][col] = (String) p.getElement().getKey();

	  if (isExternal(p)) {
		  ;
	  } else {
		  if (left(p).getElement() != null) {
			  treePicture[row+2][col-10/(depth+1)] = "/";
			  printTree1(row+4, col-14/(depth+1), depth+1, left(p), treePicture);
		  }
		  if (right(p).getElement() != null){
			  treePicture[row+2][col+10/(depth+1)] = "\\";
			  printTree1(row+4, col + 14/(depth+1), depth+1, right(p),  treePicture);
		  }
	  }
  } 
}
