...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  public int searchDepth(Position<Entry<K,V>> p, K key, int count) {
	    if (isExternal(p))
	        return -1;                          // key not found; return the final leaf
	    int comp = compare(key, p.getElement());
	    if (comp == 0)
	      return 1 + count;                          // key found; return its position
	    else if (comp < 0)
	    	return searchDepth(left(p), key, count + 1);
	    else
	    	return searchDepth(right(p), key, count + 1);  	  
  }
	  ...
	  int rightHeight = 0;
	    ...
	    if (isExternal(p)) {
	      ...
	      if (isInternal(left(p)))  
	    	  leftHeight = getHeight(left(p));
	      if (isInternal(right(p)))
	    	  rightHeight = getHeight(right(p));	
  ...
  public V put1(K key, V value) throws IllegalArgumentException {
	  dumpRecurse(root(), 0);
	  return null;
  }
