...
CA6.manifestWord <- function(sana){ # we get ourselves a string of 'r's and 'f's
...
CA6.makeDataFrame <- function() {
    r1 <- 3.7 #distance to outer pentagon
    V <- matrix(nrow = 360, ncol = 2)
    V[1:6,1] <- r1*(cos((0:5)*0.4*pi-0.5*pi))
    V[1:6,2] <- r1*(sin((0:5)*0.4*pi-0.5*pi))
    r2 <- 3 #distance to inner pentagon
    V[7:12,1] <- r2*(cos((0:4)*0.4*pi-0.5*pi))
    V[7:12,2] <- r2*(sin((0:4)*0.4*pi-0.5*pi))
    #Interpolate
    V[13:18,] <- (5*V[7:12,]+V[c((7:10),6),])/6
    V[19:24,] <- (V[7:12,]+5*V[c((7:10),6),])/6
    s <- V[13:18,]-V[7:12,]
    V[25:30,] <- V[c(17:20,16),]-s
    V[31:36,] <- V[c(15,11:14),]+s
    r3 <- 0.5 #distance to central pentagon
    V[37:42,1] <- -r3*(cos((0:4)*0.4*pi-0.5*pi))
    V[37:42,2] <- -r3*(sin((0:4)*0.4*pi-0.5*pi))
    r4 <- 1 #distance to next pentagon
    V[43:48,1] <- -r4*(cos((0:4)*0.4*pi-0.5*pi))
    V[43:48,2] <- -r4*(sin((0:4)*0.4*pi-0.5*pi))
    V[49:54,] <- V[c(37:40,36),]-s
    V[46:50,] <- V[c(43:48),]+s
    V[51:65,] <- V[c(50,46:49),]+s
    V[67:72,] <- V[c(42:45,41),]-s
    ...
    for (xarisxis in 1:60){
        DF$sana[xarisxis] <- CA6.multiply(DF$sana[xarisxis],"")
    }
...
CA6.drawGraph <- function(DF, permlabel=FALSE) { # not using the permlabel argument but leaving it in because hope springs eternal
...
CA6.power <- function(a,n){
    x <-a # it does pretty much what you'd expect
    while (n>1){
        x <- CA6.multiply(x,a) # it's multiplying
        n <- n-1 # n times
    }
    return(x)
}
...
CA6.markVertex <- function(DF,x,color) {
    if (is.character(x)){
        x <- CA6.multiply(x,"")
        x <- which(DF$sana == x)
    }
...
A5DF <- CA6.makeDataFrame() #this seems like a weird place to put this but whatever

# A5DF <- CA6.markVertex(A5DF,"frrrf","orangered")
# A5DF <- CA6.markVertex(A5DF,59,"cornflowerblue")
 CA6.drawGraph(A5DF)
...
CA6.makePerm <- function(DF,sana,IVert ="I", rSana = "(12345)", fSana = "(12)(34)"){
...
CA6.makePerms <- function(DF,rSana = "(12345)", fSana = "(12)(34)"){
    for (i in (1:nrow(DF))){
        DF$perm[i] <- CA6.makePerm(DF,DF$sana[i],"I",rSana,fSana)
...
CA6.findClosestVertex <- function(x,y){
    DF <- CA6.makeDataFrame()
    distsq <- numeric(6)
    for (i in 1:60){
...
# perms <- sapply(A5DF$sana,CA6.makePerm,DF=A5DF)
# perms
